...
USER_AGENT = (
    "AlabamaTermiteMap/0.1 "
    "(https://github.com/mizumoto-lab/AL-termite-map)"
)
...
def download_counties():
    ...
    if len(features) != 67:
        raise RuntimeError(
            f"Expected 67 Alabama counties, received {len(features)}."
        )
...
def point_in_alabama(lon, lat, counties):
    for feature in counties.get("features", []):
        ...
        if geom.get("type") == "Polygon":
            ...
            if any(point_in_polygon(lon, lat, p) for p in coords):
                return True
    ...
    while total is None or received < total:
        ...
        for obs in results:
            ...
            records.append({
                ...
                "license": obs.get("license_code") or obs.get("license") or "",
                "url": (
        ...
        print(
            f"{genus}: API page {page}, "
            f"{len(records)} Alabama observations retained."
        )
...
def main():
    ...
    parser.add_argument(
        "--refresh-counties",
        action="store_true",
        help="Download Alabama county geometry again.",
    )
    ...
    if args.refresh_counties or not COUNTY_FILE.exists():
        ...
        county_date = (
            previous.get("counties", {}).get("downloaded")
            or "existing local snapshot"
        )
    ...
    print(f"iNaturalist records: {len(records)}")
    print()
