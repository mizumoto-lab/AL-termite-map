...
The default view shows Formosan subterranean termite, published county records, and verified museum specimens. The iNaturalist layer is off by default and is loaded only when enabled.
...
For *Coptotermes formosanus*, public coordinates are displaced by up to 300 m. The displacement is deterministic for a voucher when the same local `.privacy_salt` is used. Public `locality` and `city` values are removed, `coordinate_generalized=yes`, and `privacy_radius_m=300`.

The map renders these records as translucent 300 m-radius circles with no center point. The circle is a public location-uncertainty area only. It is not the biological range, territory, colony size, or infestation extent.

Other museum records may retain exact coordinates and locality information.
...
The exact specimen master and the privacy salt must remain local/private:
...
If the repository still contains an old exact `AU-termite-samples.csv` and no secret master, `make_public_data.py` can make the initial local secret copy before replacing the public file. Once the public CSV has already been generalized, the script intentionally refuses to recreate a secret master from it.
